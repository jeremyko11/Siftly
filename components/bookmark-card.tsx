    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className={`text-sm font-semibold group-hover/link:transition-colors truncate ${headingClass}`}>
          {lt === 'github' ? pathname.replace(/^\//, '') || domain : domain}
        </p>
        <p className={`text-xs truncate mt-0.5 ${subColor}`}>
          {lt === 'github' ? `github.com${pathname}` : lt === 'cloud' ? `${domain}${pathname}` : lt === 'video' ? `${domain}${pathname}` : url}
        </p>
      </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap min-h-[1.5rem]">
            ...
            {urls.length > 0 && (
              <button
                ...
                title={urls.map(u => { try { return new URL(u).hostname } catch { return u } }).join(', ')}
              >
                <Globe size={9} />
                {urls.length === 1 ? (
                  <span className="truncate max-w-[80px]">{(() => { try { return new URL(urls[0]).hostname.replace(/^www\./, '') } catch { return 'link' } })()}</span>
                ) : (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className={`text-xs font-medium truncate ${
                        ...
                        {getDomain(url)}
                        {lt === 'github' && <span className="ml-1.5 text-orange-400/70 font-normal">{url.split('github.com')[1]}</span>}
                      </p>
                      <p className="text-[10px] text-zinc-600 truncate">{url}</p>
                    </div>
