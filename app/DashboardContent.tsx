...
function StatCard({ label, value, icon: Icon, iconColor, iconBg, borderColor, href }: {
  label: string; value: number
  icon: React.ComponentType<{ size?: number; className?: string }>
  iconColor: string; iconBg: string; borderColor: string; href?: string
}) {
  const inner = (
    <>
...
export default function DashboardContent({ data }: DashboardContentProps) {
  ...
  const greeting = isZh ? getGreetingZh() : (() => {
    const hour = new Date().getHours()
    if (hour < 12) return 'Good morning'
    if (hour < 17) return 'Good afternoon'
    return 'Good evening'
  })()
  const dateStr = isZh ? formatDateZh() : new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })
    ...
    return (
      <div className="p-6 md:p-8 min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md mx-auto">
          ...
          <h2 className="text-2xl font-bold text-zinc-100 mb-3">{t.noBookmarksYet}</h2>
          <p className="text-zinc-400 mb-8 leading-relaxed">
            {t.importYourBookmarks}
          </p>
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
            ...
            <p className="text-zinc-400 mt-1.5">
              ...
              {data.likeSourceCount > 0 && (
                <span className="text-zinc-500">
                  {' '}({data.bookmarkSourceCount.toLocaleString()} {t.bookmarks}, {data.likeSourceCount.toLocaleString()} {t.likes})
                </span>
              )}
            </p>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          label={data.likeSourceCount > 0 ? `${data.bookmarkSourceCount.toLocaleString()} ${t.bookmarks} · ${data.likeSourceCount.toLocaleString()} ${t.likes}` : t.totalBookmarks}
        ...
        <section className="pb-8">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <h2 className="text-xl font-semibold text-zinc-100">{t.topCategories}</h2>
            </div>
            <Link
              href="/categories"
              className="flex items-center gap-1.5 text-sm text-indigo-400 hover:text-indigo-300 transition-colors font-medium"
