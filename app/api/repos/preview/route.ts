...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (existing) {
      return NextResponse.json({ alreadyExists: true, fullName: existing.fullName })
    }
    ...
    if (readme && readme.trim().length > 100) {
      try {
        ...
        const text = readme.slice(0, 8000)
        ...
        console.warn('[repos/preview] AI analysis failed, returning without analysis:', aiErr)
      }
    }
    ...
    return NextResponse.json({
      ...
      topics: meta.topics,
      readmePreview: readme ? readme.replace(/[#*`_\[\]!]/g, '').slice(0, 300).trim() : null,
      analysis,
      analysisZh,
    })
  } catch (err) {
    console.error('[repos/preview]', err)
    return NextResponse.json(
